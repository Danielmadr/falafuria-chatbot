...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (lastMessage && lastMessage.role === 'assistant') {
      ...
      if (hasError) {
        setErrorState({
          message: "Ocorreu um erro na comunicação com o assistente. Por favor, tente novamente.",
          timestamp: Date.now()
        });
      }
    }
  ...
  const setPosition = useCallback((pos: Position | ((prev: Position) => Position)) => {
  ...
  const setSize = useCallback((size: Size | ((prev: Size) => Size)) => {
